...
def select_assets(assets):
    ...
    selected_options = pick(options, title, multiselect=True, min_selection_count=1)
...
def main():
    try:
        ...
        # Remove existing SELECTED_ASSETS line from .env
        with open(env_file, 'r') as f:
            lines = f.readlines()
        with open(env_file, 'w') as f:
            for line in lines:
                if not line.startswith('SELECTED_ASSETS='):
                    f.write(line)
            f.write(f'SELECTED_ASSETS="{selected_assets_str}"\n')
        print("Selected assets saved to .env")
