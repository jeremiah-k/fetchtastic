...
def test_target_manifest_legacy_cached_404_is_ignored_on_read(
    monkeypatch, tmp_path
) -> None:
    """Empty payloads cached by older versions must not be served.

    Before empty results stopped being cached, a transient 404 was pinned
    for the 24h target-manifest TTL. Those legacy entries must be treated
    as a miss on read so the live manifest is observed on the next check.
    """
    responses = iter([_FakeResponse(200, TARGET_BODY)])
    calls = 0
    ...
    # Simulate a legacy poisoned entry: empty payload, fresh timestamp.
    cache_file = cm.get_cache_file_path("firmware_nightly")
    legacy = {
        f"nightly:target:{target_id}": {
            "payload": {},
            "cached_at": datetime.now(timezone.utc).isoformat(),
        }
    }
    assert cm.atomic_write_json(cache_file, legacy)

    # Non-forced read must ignore the legacy empty payload and refetch.
    assert cm.get_nightly_target_manifest(target_id, force_refresh=False) == TARGET_BODY
    assert calls == 1
