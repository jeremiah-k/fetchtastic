...
def mock_repo_contents():
    ...
    return [
        # Directories
        {"name": "firmware-2.0", "path": "firmware-2.0", "type": "dir"},
        {"name": "firmware-1.0", "path": "firmware-1.0", "type": "dir"},
        {"name": "assets", "path": "assets", "type": "dir"},
        {"name": ".git", "path": ".git", "type": "dir"}, # Should be excluded
        # Files
        {"name": "index.html", "path": "index.html", "type": "file", "download_url": "url1"},
        {"name": "image.png", "path": "image.png", "type": "file", "download_url": "url2"},
        {"name": "README.md", "path": "README.md", "type": "file", "download_url": "url3"}, # Should be excluded
    ]
...
def test_fetch_repo_contents(mocker, mock_repo_contents):
    ...
    # Check filtering - should be 5 items (3 dirs, 2 files)
    assert len(items) == 5
    assert not any(item['name'] == '.git' for item in items)
    assert not any(item['name'] == 'README.md' for item in items)

    # Check sorting
    assert items[0]['name'] == 'firmware-2.0' # Firmware dirs sorted descending
    assert items[1]['name'] == 'firmware-1.0'
    assert items[2]['name'] == 'assets' # Other dirs sorted ascending
    assert items[3]['name'] == 'image.png' # Files sorted ascending
