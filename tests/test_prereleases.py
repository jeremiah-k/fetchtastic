...
def get_prerelease_tracking_info(
    ...
    if hasattr(cache_module, "_ensure_cache_dir"):
        ...
        tracking_file_path = _cache_manager.cache_dir + "/prerelease_tracking.json"
...
def update_prerelease_tracking(latest_release_tag, current_prerelease):
    """Legacy wrapper for updating prerelease tracking."""
    return _prerelease_manager.update_prerelease_tracking(
        latest_release_tag, current_prerelease, cache_manager=_cache_manager
    )


def get_commit_timestamp(commit_hash, github_token=None):
    return _cache_manager.get_commit_timestamp(
        "meshtastic", "Meshtastic-device", commit_hash, github_token=github_token
    )
...
def _get_commit_cache_file():
    from pathlib import Path

    return Path(_cache_manager.cache_dir) / "commit_timestamps.json"
