...
def mock_firmware_assets():
    ...
    return [
        {"name": "firmware-rak4631-2.3.2.zip"},
        {"name": "firmware-tbeam-2.3.2.zip"},
...
def test_fetch_firmware_assets(mocker, mock_firmware_assets):
    ...
    assert len(assets) == 3 # All files are kept, unlike the apk menu
    assert "firmware-rak4631-2.3.2.zip" in assets
    assert "some-other-file.txt" in assets


@pytest.mark.parametrize("filename, expected", [
    ("firmware-rak4631-2.3.2.zip", "firmware-rak4631.zip"),
...
def test_select_assets(mocker):
    ...
    assets = ["firmware-rak4631-2.3.2.zip", "firmware-tbeam-2.3.2.zip"]

    # 1. User selects one asset
    mock_pick.return_value = [("firmware-rak4631-2.3.2.zip", 0)]
...
def test_run_menu(mocker):
    ...
    mock_fetch = mocker.patch("fetchtastic.menu_firmware.fetch_firmware_assets", return_value=["asset1.zip"])
    mock_select = mocker.patch("fetchtastic.menu_firmware.select_assets", return_value={"selected_assets": ["base-pattern"]})
