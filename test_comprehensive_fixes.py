...
def test_pattern_matching():
    ...
    for file_name in prerelease_files:
        ...
        if any(exclude in file_name for exclude in exclude_patterns):
            print(f"  ❌ EXCLUDED by pattern: {[p for p in exclude_patterns if p in file_name]}")
        ...
        matching_patterns = [pattern for pattern in extract_patterns if pattern in stripped_file_name]
        if matching_patterns:
            print(f"  ✅ MATCHED by pattern: {matching_patterns}")
            matched_files.append(file_name)
        else:
            print(f"  ⏭️  SKIPPED - no matching patterns")
...
if __name__ == "__main__":
    ...
    if overall_success:
        ...
        print("   - Pre-release downloads will use EXTRACT_PATTERNS instead of SELECTED_FIRMWARE_ASSETS")
        print("   - Exclude patterns will be properly applied to pre-release downloads")
        print("   - Latest versions will be correctly displayed (v2.6.9.f223b8a for firmware, 2.6.9 for Android)")
